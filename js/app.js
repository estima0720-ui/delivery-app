...
function createApiKeyConfigPanel() {
  ...
  // Z-indexを付与し、他の要素より手前に確実に描画されるようにスタイルを調整
  panel.style = "background: #fff3cd; padding: 10px 15px; border-bottom: 2px solid #ffeeba; display: flex; justify-content: space-between; align-items: center; font-family: sans-serif; font-size: 13px; color: #856404; box-shadow: 0 2px 5px rgba(0,0,0,0.05); position: relative; z-index: 9999;";
  
  let currentKey = "";
  try {
    currentKey = localStorage.getItem("user_gemini_api_key") || "";
  } catch (e) {
  ...
  document.getElementById("saveKeyBtn").addEventListener("click", () => {
    ...
    try {
      ...
      alert("ブラウザの設定によりAPIキーを保存できませんでした。");
    }
  });
...
async function openMiniMap(fileName) {
  ...
  modal.innerHTML = `
    <div style="background: white; width: 95%; max-width: 950px; padding: 20px; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); position: relative; font-family: sans-serif; text-align: center; box-sizing: border-box;">
      ...
      <div style="padding: 50px 0;">
        ...
        <div style="font-size: 14px; color: #666; margin-bottom: 5px;">「001_台帳」等のフォルダ内から、この店舗の進入図（画像・PDF）を探しています。</div>
        <div style="font-size: 12px; color: #999;">※1万件以上のファイルがあっても、独自の検索システムにより高速で完了します。</div>
      </div>
  ...
  try {
    ...
    if (foundFiles && foundFiles.length > 0) {
      ...
      const previewItems = await Promise.all(
        filesToShow.map(async (file) => {
          try {
            ...
            return { id: file.id, name: file.name, url, isPdf, webViewLink };
          } catch (e) {
            console.error(`ファイル取得失敗: ${file.name}`, e);
            return null;
          }
        })
      );
      ...
      let previewHtml = `
        <div style="display: flex; gap: 12px; width: max-content; min-width: 100%; height: 100%; justify-content: flex-start; align-items: stretch; flex-wrap: nowrap; box-sizing: border-box;">
      `;
      ...
      validPreviews.forEach(item => {
        ...
        const fileUrl = item.webViewLink;

        previewHtml += `
          <div style="width: ${itemWidth}; min-width: 280px; max-width: 100%; flex-shrink: 0; display: flex; flex-direction: column; border: 1px solid #ccc; border-radius: 6px; background: #fff; padding: 6px; box-sizing: border-box; height: 100%;">
            <a href="${fileUrl}" target="_blank" rel="noopener noreferrer" style="font-size: 12px; font-weight: bold; text-overflow: ellipsis; overflow: hidden; white-space: nowrap; margin-bottom: 6px; color: #1a73e8; text-decoration: underline; text-align: left; border-bottom: 1px solid #eee; padding-bottom: 4px; display: block; cursor: pointer;" title="ファイルを別タブで開く: ${escapeHtml(item.name)}">
      ...
      modal.innerHTML = `
        <div style="background: white; width: 95%; max-width: 950px; padding: 15px 20px; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); position: relative; font-family: sans-serif; box-sizing: border-box;">
          <h3 style="margin-top: 0; color: #1e50a2; font-size: 18px; border-bottom: 2px solid #1e50a2; padding-bottom: 8px; display: flex; justify-content: space-between; align-items: center; text-align: left;">
            ...
            <span style="font-size: 11px; background: #2e7d32; color: white; padding: 3px 8px; border-radius: 12px; font-weight: bold;">🟢 ドライブ連携OK</span>
          </h3>
          <p style="font-size: 12px; color: #555; margin-bottom: 10px; text-align: left;">
            キーワード「<b>${escapeHtml(searchKeyword)}</b>」の図面を並べて表示しています。<b>スマホでは左右にスワイプ（スクロール）して残りの図面を確認できます。</b> 各ファイル名タップでファイルを開くことも可能です。
          </p>
          
          <div id="miniMapContainer" style="width: 100%; height: 460px; background: #fcfcfc; border-radius: 4px; overflow-x: auto; overflow-y: hidden; border: 1px solid #ccc; padding: 8px; box-sizing: border-box; -webkit-overflow-scrolling: touch;">
            ${previewHtml}
          </div>
          ...
          <button onclick="document.getElementById('miniMapModal').remove()" style="position: absolute; top: 15px; right: 20px; background: none; border: none; font-size: 20px; cursor: pointer; color: #999;">✕</button>
          <div style="margin-top: 15px; text-align: right; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
            <span style="font-size: 11px; color: #888; text-align: left; display: block;">※ 図面がない場合は、右のボタンからGoogleマップに切り替えられます。</span>
            <div style="display: flex; gap: 8px; width: 100%; justify-content: flex-end;">
              <button onclick="switchToGoogleMap('${escapeJSString(fileName)}')" style="background-color: #34a853; color: white; border: none; padding: 8px 15px; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold;">🗺 Googleマップを表示</button>
              <button onclick="document.getElementById('miniMapModal').remove()" style="background-color: #7f8c8d; color: white; border: none; padding: 8px 15px; border-radius: 4px; cursor: pointer; font-size: 13px;">閉じる</button>
            </div>
          </div>
        </div>
      `;
...
async function runGeminiOCR(base64DataUrl) {
  ...
  const prompt = `
    ...
    2. 車両ナンバー (vehicleNumber):
       ...
       - OCRの特性上、稀にカッコの片方が見えにくくなっていたり、類似の記号「( )」「{ }」「【 】」などに誤認識されているケースがありますが、車番項目に直結する部分であれば柔軟に補正して車番を特定してください。
       - 該当する情報がどうしても見つからない場合は、空文字 "" としてください。
  `;
