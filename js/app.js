...
// ==========================================
// 10. 【スマホ完全対応・リンク化】周辺図・ルート図（横スクロール & タップ移動対応）
// ==========================================
async function openMiniMap(fileName) {
  ...
  try {
    ...
    if (foundFiles && foundFiles.length > 0) {
      ...
      // 横幅の比率計算（1件: 100%, 2件: 49%, 3件: 32%）
      // スマホでは min-width: 280px と flex-shrink: 0 で綺麗に横並びスクロールします
      ...
      // ★ 修正点：親コンテナ（#miniMapContainer）に overflow-x: auto とタッチジェスチャ対応を指定
      modal.innerHTML = `
...
async function runGeminiOCR(base64DataUrl) {
  ...
  // ★ 修正点：車番項目（D [46-23] 等）を最優先・高精度で読み取るようプロンプトを大幅改良
  const prompt = `
